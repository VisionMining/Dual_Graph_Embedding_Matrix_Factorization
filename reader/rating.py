...
class RatingGetter(object):
    ...
    def __init__(self):
        ...
        self.validSet_u = defaultdict(dict)  # used to store the test set by hierarchy user:[item,rating]
        self.validSet_i = defaultdict(dict)
    ...
    def generate_data_set(self):
        ...
        self.all_tv_User.update(self.user)
        self.all_tv_Item.update(self.item)
        self.all_tt_User.update(self.user)
        self.all_tt_Item.update(self.item)
        self.id2all_User.update(self.id2user)
        self.id2all_Item.update(self.id2item)
        ...
        for index, line in enumerate(self.validSet()):
            ...
            if not u in self.user:
                ...
                self.all_tv_User[u] = len(self.all_tv_User)
                self.id2all_User[self.all_User[u]] = u
            if not i in self.item:
                self.all_Item[i] = len(self.all_Item)
                self.all_tv_Item[i] = len(self.all_tv_Item)
                self.id2all_Item[self.all_Item[i]] = i
            if not u in self.user_valid:
                self.user_valid[u] = len(self.user_valid)
                self.id2user_valid[self.user_valid[u]] = u
            if not i in self.item_valid:
                self.item_valid[i] = len(self.item_valid)
                self.id2item_valid[self.item_valid[i]] = i
            self.validSet_u[u][i] = r
            self.validSet_i[i][u] = r
            self.validSetLength = index + 1
        ...
        for index, line in enumerate(self.testSet()):
            ...
            if not u in self.user:
                # self.all_User[u] = len(self.all_User)
                self.all_tt_User[u] = len(self.all_tt_User)
            if not i in self.item:
                # self.all_Item[i] = len(self.all_Item)
                self.all_tt_Item[i] = len(self.all_tt_Item)
            if not u in self.all_User:
                self.all_User[u] = len(self.all_User)
                self.id2all_User[self.all_User[u]] = u
            if not i in self.all_Item:
                self.all_Item[i] = len(self.all_Item)
                self.id2all_Item[self.all_Item[i]] = i
            if not u in self.user_test:
                self.user_test[u] = len(self.user_test)
                self.id2user_test[self.user_test[u]] = u
            if not i in self.item_test:
                self.item_test[i] = len(self.item_test)
                self.id2item_test[self.item_test[i]] = i
    ...
    def validSet(self):
        with open(self.config.rating_valid_path, 'r') as f:
            for index, line in enumerate(f):
                u, i, r = line.strip('\r\n').split(self.config.sep)
                # if float(r) >= 3:
                #     r = 1
