...
if __name__ == '__main__':
    ...
    maes = []
    gemf = SVDPP()
    gemf.config.k_fold_num = 1
    for i in range(gemf.config.k_fold_num):
        print('the %d-th cross validation training' % i)
        gemf.train_model(i)
        rmse, mae = gemf.predict_model()
        rmses.append(rmse)
        maes.append(mae)
    rmse_avg = sum(rmses) / gemf.config.k_fold_num
    mae_avg = sum(maes) / gemf.config.k_fold_num
